 ...
 */
public class PointQuadTree<V> extends QuadTree<com.github.tagwanj.math.Vector3, V> {

	public PointQuadTree(float minX, float minZ, float maxX, float maxZ) {
		super(minX, minZ, maxX, maxZ);
	}
	
	public PointQuadTree(com.github.tagwanj.math.Vector3 startPoint, com.github.tagwanj.math.Vector3 endPoint) {
		this(startPoint.x, startPoint.z, endPoint.x, endPoint.z);
	}

	@SuppressWarnings({ "unchecked", "rawtypes" })
	@Override
	public void set(com.github.tagwanj.math.Vector3 k, V v) {
	...
	@Override
	public V get(com.github.tagwanj.math.Vector3 k, V defaultValue) {
		Node<V> node = this.find(this.root, k);
		return node != null ? node.getData().getValue() : defaultValue;
	}

	@Override
	public Node<V> find(Node<V> node, com.github.tagwanj.math.Vector3 vector3) {
		...
		case LEAF:
			com.github.tagwanj.math.Vector3 oldPoint = ((PointData<V>) node.getData()).getPoint();
	...
	@Override
	public V remove(com.github.tagwanj.math.Vector3 k) {
	...
	public List<com.github.tagwanj.math.Vector3> getKeys() {
		final List<com.github.tagwanj.math.Vector3> arr = new ArrayList<com.github.tagwanj.math.Vector3>();
		this.traverse(this.root, new Func<V>() {
			@Override
			public void call(PointQuadTree<V> quadTree, Node<V> node) {
